...
class OrbisDB:
    ...
    def add_row(self, entry_data):
      ...
      if not self.controller:
          raise ValueError("Read-only database. OrbisDB controller has not been specified. Cannot write to the database.")
  
      if not self.context_stream:
          raise ValueError("A context stream must be provided when creating streams in a shared instance.")
    ...
    def filter(self, env_id: str, filters):
      ...
      filter_list = [f"{key} = '{value}'" if isinstance(value, str) else f"{key} = {value}" for key, value in filters.items()]  # strings need to be wrapped around single quotes
